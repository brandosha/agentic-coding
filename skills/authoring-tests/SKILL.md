...
- **task.yaml**: Identify the specific files and entities in scope, the tests to author, and the completion criteria.
...
Based on the test definitions and implementation guidance in `task.yaml`:
...
## 5. Handling Blockers
If you find that the code is "untestable" in its current state (e.g., extreme coupling not noted in Discovery) or if the `task.yaml` test requirements are logically impossible:
...
## 6. Progress & Memory
- Keep your test files organized within the project's standard test directory (or as specified in `task.yaml`).
