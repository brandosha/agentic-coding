...
Your primary domain is the `4-development/` stage. You are invoked by the Task Lifecycle Manager and report findings back to the Manager (not the Human).
...
- **Scope Discipline:** Do NOT modify files or functions not listed in the `task.yaml` unless you discover a critical dependency (in which case, move to `8-blocked` or seek Manager refinement).
...
3. Report completion to the Manager; the Manager moves the task folder to `5-review/`.
