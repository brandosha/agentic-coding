...
function main(taskId) {
  ...
  // Check for uncommitted changes, then remove the worktree
  ...
  if (task.phase !== 'done') {
    console.error(`Error: Task ${taskId} is in phase "${task.phase}", ensure the task is marked as "done" in task.json before cleanup.`);
    process.exit(1);
  }
...
if (!taskId) {
  console.error('Error: Missing task ID argument. Usage: node complete-task.js <task-id>');
  process.exit(1);
}
