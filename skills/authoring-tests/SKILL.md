...
1. Write **Pinning Tests** (Characterization Tests) that reflect the *current* behavior of the entities listed in `task.json`.
...
## 5. Handling Blockers
If you find that the code is "untestable" in its current state (e.g., extreme coupling not noted in Discovery) or if the `task.json` test requirements are logically impossible:
...
## 6. Progress & Memory
- Keep your test files organized within the project's standard test directory (or as specified in `task.json`).
